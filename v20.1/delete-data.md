...
1. Using a `WHERE` clause to limit the number of rows:
    {% include copy-clipboard.html %}
    ~~~ sql
    DELETE FROM student_loan_accounts WHERE loan_amount < 30000;
    ~~~

2. Using a multi-row statement:
    {% include copy-clipboard.html %}
    ~~~ sql
...
- If you issue multiple [`DELETE`](delete.html) statements that delete large amounts of data, each `DELETE` statement will run more and more slowly, for reasons [explained in this FAQ entry](sql-faqs.html#why-are-my-deletes-getting-slower-over-time).
